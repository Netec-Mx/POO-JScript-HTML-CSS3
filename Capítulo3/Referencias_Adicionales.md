

# Referencias adicionales

<br/><br/>

* [MDN HTML Overview](https://developer.mozilla.org/en-US/docs/Web/HTML)
* [MDN Getting Started with HTML](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/Structuring_content/Basic_HTML_syntax)
* [WHATWG HTML Living Standard](https://html.spec.whatwg.org/)
* [W3C HTML Tutorial](https://www.w3.org/TR/html52/)
* [MDN Semantic HTML](https://developer.mozilla.org/en-US/docs/Glossary/Semantics#semantics_in_html)
* [MDN HTML Sections and Outlines](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/section)
* [MDN article Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/article)
* [WHATWG Sections](https://html.spec.whatwg.org/multipage/sections.html)
* [MDN nav Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/nav)
* [MDN anchor Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a)
* [MDN URLs and Links](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/Structuring_content/Creating_links)
* [WHATWG Links](https://html.spec.whatwg.org/multipage/links.html)
* [MDN img Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img)
* [MDN audio Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio)
* [MDN video Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/video)
* [MDN Multimedia and Embedding](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/Structuring_content/HTML_video_and_audio_content)
* [WHATWG Embedded Content](https://html.spec.whatwg.org/multipage/embedded-content.html)
* [MDN table Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/table)
* [MDN th Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/th)
* [MDN td Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/td)
* [MDN HTML Tables Basics](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/Structuring_content/HTML_table_basics)
* [WHATWG Tables](https://html.spec.whatwg.org/multipage/tables.html)
* [MDN form Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form)
* [MDN input Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input)
* [MDN label Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/label)
* [MDN HTML Forms Guide](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Forms)
* [WHATWG Forms](https://html.spec.whatwg.org/multipage/forms.html)
* [MDN aside Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/aside)
* [MDN footer Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/footer)
* [MDN header Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/header)
* [WHATWG Sections and Grouping Content](https://html.spec.whatwg.org/multipage/sections.html)
* [MDN details Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details)
* [MDN summary Element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/summary)
* [MDN Interactive Elements](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details#creating_an_open_disclosure_box)
* [WHATWG Interactive Elements](https://html.spec.whatwg.org/multipage/interactive-elements.html)
* [Can I Use](https://caniuse.com/)
* [MDN Browser Compatibility](https://developer.mozilla.org/en-US/docs/MDN/Writing_guidelines/Page_structures/Compatibility_tables)
* [W3C Markup Validation Service](https://validator.w3.org/)
* [Nu HTML Checker](https://validator.w3.org/nu/)
* [MDN Learn Web Development](https://developer.mozilla.org/en-US/docs/Learn_web_development)
* [web.dev HTML Course](https://web.dev/learn/html/)
* [HTML 5 Test](https://html5test.com/)
* [HTML/CSS/JavaScript Online](https://codepen.io/pen/)

* [Unsplash](https://unsplash.com/) — Fotografías gratuitas de alta calidad
* [Pexels](https://www.pexels.com/) — Imágenes y videos gratuitos
* [Pixabay](https://pixabay.com/) — Imágenes, videos, música y efectos de sonido
* [Freepik](https://www.freepik.com/) — Recursos gráficos e ilustraciones (algunos requieren atribución)
* [Openverse](https://openverse.org/) — Buscador abierto de imágenes y audio con licencias Creative Commons
* [Wikimedia Commons](https://commons.wikimedia.org/) — Multimedia libre y educativa
* [Burst by Shopify](https://burst.shopify.com/) — Fotos gratuitas para proyectos web


<br/>
<br/>

## Audio / Música / Sonidos

* [Freesound](https://freesound.org/) — Efectos de sonido y audio colaborativo
* [Mixkit Audio](https://mixkit.co/free-stock-music/) — Música gratuita y efectos de sonido
* [YouTube Audio Library](https://studio.youtube.com/channel/UC/music) — Música y sonidos libres para proyectos
* [Bensound](https://www.bensound.com/) — Música libre con distintas licencias
* [Incompetech](https://incompetech.com/music/) — Música gratuita de Kevin MacLeod


<br/>
<br/>

## Video

* [Pexels Videos](https://www.pexels.com/videos/)
* [Pixabay Videos](https://pixabay.com/videos/)
* [Mixkit Videos](https://mixkit.co/free-stock-video/)
* [Videvo](https://www.videvo.net/) — Clips y motion graphics gratuitos
* [Coverr](https://coverr.co/) — Videos gratuitos para páginas web

<br/>
<br/>

## Íconos y SVG

* [Font Awesome](https://fontawesome.com/)
* [Heroicons](https://heroicons.com/)
* [SVG Repo](https://www.svgrepo.com/)
* [Flaticon](https://www.flaticon.com/)

<br/>
<br/>

## Recomendación para el curso

Para demos HTML5 básicas normalmente basta con:

* Imágenes → [Unsplash](https://unsplash.com/) o [Pexels](https://www.pexels.com/)
* Audio → [Mixkit Audio](https://mixkit.co/free-stock-music/)
* Video → [Pexels Videos](https://www.pexels.com/videos/)
* Íconos → [SVG Repo](https://www.svgrepo.com/)

